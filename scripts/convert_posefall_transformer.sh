...
REFERENCE="${REFERENCE:-${ROOT}/exports/posefall-svp/${RUN_NAME}/svp/calibration/head_reference.npz}"
IMAGE_LIST="${IMAGE_LIST:-${ROOT}/exports/posefall-svp/${RUN_NAME}/svp/calibration/head_input_real.txt}"
OUT_DIR="${OUT_DIR:-${ROOT}/exports/posefall-svp/${RUN_NAME}/picovision}"

mkdir -p "${OUT_DIR}/model"

"${PYTHON}" "${ROOT}/scripts/export_posefall_picovision.py" \
  --extract \
  --weights "${WEIGHTS}" \
  --reference "${REFERENCE}" \
...
atc \
  ...
  --output="${OUT_DIR}/model/posefall_head_picovision" \
...
echo "Transformer OM: ${OUT_DIR}/model/posefall_head_picovision_original.om"
