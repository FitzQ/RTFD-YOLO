...
FALL_KEYPOINT_DIM = 51
FALL_FEATURE_DIM = 56
FALL_CENTER_Y_INDEX = FALL_KEYPOINT_DIM
...
class FallTransformer(nn.Module):
    """Transformer head for video-level fall classification with internal sliding-window pooling."""
...
def normalize_keypoints(
    ...
    feature_dim: int = FALL_FEATURE_DIM,
) -> torch.Tensor:
    ...
    for i in range(out.shape[0]):
        ...
        if feature_dim == FALL_KEYPOINT_DIM:
            features.append(rel)
            continue
...
def mean_keypoint_confidence(seq: torch.Tensor) -> float:
    ...
    if seq.numel() == 0:
        return 0.0
    return float(seq[..., :FALL_KEYPOINT_DIM].reshape(-1, 17, 3)[..., 2].mean().item())


def load_fall_head(weights: str | None, device: torch.device | str, input_dim: int = FALL_FEATURE_DIM, window: int = 60):
    """Load a fall head checkpoint, or return None when no weights are provided."""
    if not weights:
    ...
    try:
        ...
        raise RuntimeError(
            f"Fall head weights at {weights!r} are incompatible with the current fall feature head. "
            "Retrain with `yolo fall train ...` to generate a new checkpoint."
        ) from e
