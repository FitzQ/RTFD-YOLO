...
from .model import (
    ...
    POSEGFALL_FEATURE_VERSION,
    PoseSegFallTransformer,
    mean_poseg_confidence,
    poseg_result_features,
)
...
def _valid_feature_payload(payload: dict, feature_dim: int) -> bool:
    return (
        ...
        and int(payload.get("feature_version", 0)) == POSEGFALL_FEATURE_VERSION
...
def _feature_model_key(model_path: str | Path | None) -> str:
    """Return a stable cache folder name for the pose model used to extract posegfall features."""
    stem = Path(str(model_path or "yolo26n-poseg.pt")).stem
    return "".join(c if c.isalnum() or c in {"-", "_", "."} else "_" for c in stem)


def posegfall_feature_settings(args, feature_dim: int) -> dict:
    """Return settings that change cached PoseSegFall front-end tensors."""
    return {
        "version": POSEGFALL_FEATURE_VERSION,
...
def _extract_tracking_features(args, posegfall_feature_dir: Path) -> None:
    model_path = getattr(args, "model", "yolo26n-poseg.yaml")
    ...
    feature_dim = int(getattr(args, "posegfall_feature_dim", POSEGFALL_FEATURE_DIM))
    workers = max(1, int(getattr(args, "fall_extract_workers", 20)))

    LOGGER.info(
        f"Extracting tracked keypoint features to {posegfall_feature_dir} "
    ...
    desc = f"Extracting posegfall features ({_feature_model_key(model_path)})"
    if workers == 1:
        for _ in TQDM(
            _extract_video_chunk(
                items, posegfall_feature_dir, model_path, device, imgsz, vid_stride, conf, iou, tracker, overwrite, min_track_len, min_conf, feature_dim
            ),
            total=len(items),
            desc=desc,
            unit="video",
...
def _extract_video_chunk(
    ...
    pose_model = YOLO(model_path, task="poseg")
    for item in items:
        _extract_one_video(
            pose_model, item, posegfall_feature_dir, device, imgsz, vid_stride, conf, iou, tracker, overwrite, min_track_len, min_conf, feature_dim
        )
        yield item
...
def _extract_one_video(
    ...
    out_path = feature_cache_path(posegfall_feature_dir, class_name, video)
    ...
    LOGGER.debug(f"Extracting tracked PoseSeg tensors from {video}...")
    ...
    for track_id, frames in tracks.items():
        ...
        mean_conf = mean_poseg_confidence(seq)
    ...
    atomic_torch_save(
        {
            ...
            "label": label,
            "feature_dim": feature_dim,
            "feature_version": POSEGFALL_FEATURE_VERSION,
        },
        out_path,
    )
    LOGGER.debug(
...
def _load_track_sequences(
    ...
    min_conf: float = 0.2,
    feature_dim: int = POSEGFALL_FEATURE_DIM,
    ...
    for path in items:
        ...
        if not _valid_feature_payload(payload, feature_dim):
            LOGGER.warning(
                f"Skipping incompatible posegfall feature cache while loading: {path} "
                f"feature_dim={payload.get('feature_dim')} version={payload.get('feature_version')} "
                f"expected_dim={feature_dim} expected_version={POSEGFALL_FEATURE_VERSION}"
            )
            continue
        ...
        if (
            seq.shape[1] == feature_dim
            and seq.shape[0] == frame_indices.shape[0]
            and seq.shape[0] >= min_track_len
            and mean_poseg_confidence(seq) >= min_conf
        ):
            sequences.append(
                TrackSequence(
                    label=label,
...
class PoseSegFallTrackDataset(Dataset):
...
class PoseSegFallTrainer(FallTrainer):
    """Native Ultralytics trainer for the frozen PoseSeg front end and PoseSegFall temporal head."""

    task_name = "posegfall"
    head_attr = "posegfall_head"
    config_attr = "posegfall_config"
    frontend_type = PoseSegModel

    @property
    def validator_class(self):
        from .val import PoseSegFallValidator

        return PoseSegFallValidator

    def build_fall_datasets(self):
        feature_dim = int(self.args.posegfall_feature_dim)
        min_track_len = int(self.args.fall_min_track_len)
        min_conf = float(self.args.fall_min_conf)
        target_fps = float(self.args.fall_target_fps)
        max_gap_seconds = float(self.args.fall_max_gap_seconds)
        settings = posegfall_feature_settings(self.args, feature_dim)
        root = _path_arg(self.args.posegfall_feature_dir, "runs/posegfall/features")
        key = self.args.posegfall_feature_key or feature_cache_key(self.args.model, "posegfall", settings)
        self.args.posegfall_feature_key = key
    ...
    def build_fall_head(self):
        return PoseSegFallTransformer(
            input_dim=int(self.args.posegfall_feature_dim),
    ...
    def complete_config(self):
        ...
        return {
            ...
            "stride": head.stride,
            "feature_version": POSEGFALL_FEATURE_VERSION,
            "feature_key": self.args.posegfall_feature_key,
            "feature_type": "box_keypoints_mask_grid",
