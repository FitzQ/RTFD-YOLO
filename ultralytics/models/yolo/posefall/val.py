...
from .model import POSEFALL_FEATURE_DIM, mean_keypoint_confidence, pose_result_features
from .train import _extract_one_video, _load_track_sequences, posefall_feature_settings
...
def _video_items_from_args(args, limit: int = 0) -> list[VideoItem]:
    ...
    if not data:
        raise ValueError(
            "posefall val requires fall_val_fall_data=/path/to/Fall "
...
class PoseFallValidator(FallValidator):
    ...
    def __call__(self, trainer=None, model=None):
        ...
        pose_model = fall_yolo_model(model, getattr(self.args, "model", "posefall26n.pt"), task="pose")
        torch_device = fall_device(device)
        posefall_head = getattr(pose_model.model, "posefall_head", None)
        if posefall_head is None:
            raise ValueError("posefall val requires complete PoseFall weights containing `posefall_head`")
        posefall_head = posefall_head.float().to(torch_device).eval()
        config = getattr(pose_model.model, "posefall_config", {})
        ...
        settings = posefall_feature_settings(self.args, expected_dim)
        feature_key = resolve_feature_cache_key(
            self.args, pose_model, "posefall", "posefall_feature_key", settings
        )
        self.args.posefall_feature_key = feature_key
        feature_dir = Path(getattr(self.args, "posefall_feature_dir", None) or "runs/posefall/features") / feature_key
        end2end = bool(getattr(self.args, "fall_val_end2end", True))
        if end2end:
            LOGGER.info("Running PoseFall end-to-end validation from decoded videos")
        else:
            LOGGER.info(f"Using shared PoseFall feature cache: {feature_dir}")
        ...
        description = "PoseFall end-to-end val" if end2end else "PoseFall cached val"
        ...
        if end2end:
            LOGGER.info(
                f"posefall end-to-end speed: frames={processed_frames} time={processing_seconds:.3f}s "
    ...
    def _predict_video_end2end(
        ...
        for track_id, frames in tracks.items():
            ...
            for sequence, _ in segments:
                confidence = mean_keypoint_confidence(sequence)
                if len(sequence) >= min_track_len and confidence >= min_conf:
                    candidates.append((len(sequence), confidence, sequence))
        if not candidates:
            reason = "decode_failed_or_empty_video" if not frames_seen else f"no_valid_track keypoint_frames={keypoint_frames}"
            return None, reason, frames_seen
        candidates.sort(key=lambda value: (value[0], value[1]), reverse=True)
        sequence = candidates[0][2]
        with torch.no_grad():
            probability = float(posefall_head(sequence.to(device)).sigmoid().item())
        window, stride = int(posefall_head.window), int(posefall_head.stride)
    ...
    def _predict_video_cached(
        ...
        window = int(getattr(posefall_head, "window", getattr(self.args, "fall_window", 60)))
        stride = int(getattr(posefall_head, "stride", getattr(self.args, "fall_stride", 15)))
    ...
    def _compute_metrics(rows: list[dict], threshold: float, elapsed: float) -> dict:
        return video_classification_metrics(rows, threshold, elapsed, PoseFallValidator._competition_metrics)

    @staticmethod
    def _competition_metrics(targets: list[int], probs: list[float]) -> dict[str, float]:
        points = PoseFallValidator._curve_points(targets, probs)
        p90 = PoseFallValidator._operating_point(points, 0.90)
        p95 = PoseFallValidator._operating_point(points, 0.95)
        return {
            "p90": p90["precision"],
            "p95": p95["precision"],
    ...
    def _save_metrics(self, rows: list[dict]) -> None:
        ...
        LOGGER.info(f"posefall val metrics saved to {out} and {curve_out}")
    ...
    def _save_competition_plots(self, rows: list[dict]) -> None:
        ...
        points = self._curve_points(targets, probs)
        if not points:
            LOGGER.warning("posefall val plots skipped: no predictions")
            return
        p90 = self._operating_point(points, 0.90)
        p95 = self._operating_point(points, 0.95)
        deployment = self._metrics_at_threshold(targets, probs, float(self.metrics["threshold"]))
        ...
        ax.set(
            ...
            xlim=(0, 1.01),
            ylim=(0, 1.01),
            title=f"PoseFall PR Curve | Competition Score={self.metrics['competition_map']:.2f}",
        )
        ax.grid(alpha=0.25)
        ...
        ax.set(
            ...
            xlim=(0, 1),
            ylim=(0, 1.01),
            title="PoseFall Metrics vs Threshold",
        )
        ax.grid(alpha=0.25)
        ax.legend(loc="best")
        fig.tight_layout()
        ...
        fig.suptitle("PoseFall Video-Level Confusion Matrices")
        fig.tight_layout()
        ...
        ax.set(
            ...
            ylabel="Video Count",
            xlim=(0, 1),
            title="PoseFall Probability Distribution",
        )
        ...
        plt.close(fig)
        LOGGER.info(f"posefall val plots saved to {save_dir}")
