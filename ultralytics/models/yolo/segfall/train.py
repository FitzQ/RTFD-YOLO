...
from .model import (
    SEGFALL_FEATURE_DIM,
    SEGFALL_FEATURE_TYPE,
    SEGFALL_FEATURE_VERSION,
    SegFallTransformer,
...
SEGFALL_RAW_FEATURE_VERSION = SEGFALL_FEATURE_VERSION
SEGFALL_RAW_FEATURE_TYPE = SEGFALL_FEATURE_TYPE
...
def segfall_feature_settings(args) -> dict:
    """Return settings that change cached SegFall front-end tensors."""
    return {
        "version": SEGFALL_RAW_FEATURE_VERSION,
...
def _extract_one_video(
    ...
    out_path = feature_cache_path(segfall_feature_dir, class_name, video)
    ...
    for track_id, raw_frames in tracks.items():
        ...
        if mean_conf >= min_conf:
            ...
            candidates.append((duration, mean_conf, track_id, raw_frames, frame_indices))
    if not candidates:
        LOGGER.debug(
            f"Skipping {video}: no tracked segmentation sequence reached fall_min_track_len={min_track_len} "
    ...
    LOGGER.debug(
        f"Saved {out_path} selected_track_span={selected_len} observations={len(raw_sequence)} "
...
def _load_track_sequences(
    ...
    for path in items:
        ...
        sparse_seq, sparse_indices = _raw_sequence_to_features(payload["raw_sequence"], feature_dim)
...
class SegFallTrainer(FallTrainer):
    """Native Ultralytics trainer for the frozen Segment front end and SegFall temporal head."""

    task_name = "segfall"
    head_attr = "segfall_head"
    config_attr = "segfall_config"
    frontend_type = SegmentationModel

    @property
    def validator_class(self):
        from .val import SegFallValidator

        return SegFallValidator

    def build_fall_datasets(self):
        feature_dim = int(self.args.segfall_feature_dim)
        ...
        root = _path_arg(self.args.segfall_feature_dir, "runs/segfall/features")
        key = self.args.segfall_feature_key or feature_cache_key(self.args.model, "segfall", settings)
        self.args.segfall_feature_key = key
        feature_dir = root / key
        expected = _limit_items(_video_items_from_args(self.args), int(self.args.fall_limit))
        cached_items = cached_feature_paths(feature_dir, expected, _valid_feature_payload)
    ...
    def build_fall_head(self):
        return SegFallTransformer(
            input_dim=int(self.args.segfall_feature_dim),
    ...
    def complete_config(self):
        ...
        return {
            ...
            "stride": head.stride,
            "feature_version": SEGFALL_RAW_FEATURE_VERSION,
            "feature_key": self.args.segfall_feature_key,
            "feature_type": SEGFALL_RAW_FEATURE_TYPE,
